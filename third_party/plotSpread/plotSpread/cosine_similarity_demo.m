function [handles,xyPos] = plotSpread(varargin)
...
% 
%       MMsize      : Size of Median / Mean markers. 
%       MarkerSize  : Size of datapoint Markers. 
% 
% 
% 
% 
    ...
    switch opt.xyOri
        case 'normal'
            if any(opt.showMM==[1,3])
                mdh = plot(ah,opt.xValues,md,'sg','MarkerSize',12,'LineWidth',lw);
            end
            if any(opt.showMM==[1,2])
                mh = plot(ah,opt.xValues,m,'+r','Color','r','MarkerSize',opt.MMsize,'LineWidth',lw);
            end
            if opt.showMM == 4
                mh = plot(ah,opt.xValues,m,'+r','Color','r','MarkerSize',opt.MMsize,'LineWidth',lw);
        ...
        case 'flipped'
            if any(opt.showMM==[1,3])
                mdh = plot(ah,md,opt.xValues,'sg','MarkerSize',opt.MMsize,'LineWidth',lw);
            end            
            if any(opt.showMM==[1,2])
                mh = plot(ah,m,opt.xValues,'+r','Color','r','MarkerSize',opt.MMsize,'LineWidth',lw);
            end
            if opt.showMM == 4
                mh = plot(ah,m,opt.xValues,'+r','Color','r','MarkerSize',opt.MMsize,'LineWidth',lw);
